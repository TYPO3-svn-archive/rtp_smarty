...
// Include rtp_smarty plugin helper functions
require_once(t3lib_extMgm::extPath('rtp_smarty').'/class.tx_rtpsmarty_div.php');

// Create a wrapper class extending Smarty
class tx_rtpsmarty_wrapper extends Smarty {
	...
	var $t3_smartyVars = array(
		...
		'compile_dir' => 0,						// String: Name of the directory where compiled templates are located (must be either a relative or absolute path)
	...
	function setSmartyVar($smartyVar,$smartyValue) {
		...
		if(is_array($this->{$smartyVar}) || is_array($smartyValue)) {
			...
			$smartyValue = array_map(array('tx_rtpsmarty_div','booleanize'),$smartyValue);
			if($this->t3_smartyVars[$smartyVar]) {
				$smartyValue = array_map(array('tx_rtpsmarty_div','getFileAbsName'),$smartyValue);
			}
			$this->{$smartyVar} = array_merge_recursive($this->{$smartyVar},$smartyValue);
		} else {
			if($this->t3_smartyVars[$smartyVar]) {
				$smartyValue = tx_rtpsmarty_div::getFileAbsName($smartyValue);
			}
			$this->{$smartyVar} = tx_rtpsmarty_div::booleanize($smartyValue);
		}
		return true;
	}
	...
	function display($resource_name, $cache_id = null, $compile_id = null) {
		...
		$_t3_fetch_result = $this->fetch($resource_name, tx_rtpsmarty_div::getCacheID($cache_id), $compile_id);
	...
	function setPathToLanguageFile($param) {
		...
		} elseif($langFile = t3lib_div::getFileAbsName($param) && @is_file($langFile)) {
			$this->t3_languageFile = $langFile;
		}
	}
	...
	function _getResourceInfo($resource_name) {
		$this->_debug_resource['dirname'] = (!dirname($resource_name) || preg_match('%^[\\\\|/|.]$%m', dirname($resource_name)))?$this->template_dir:dirname($resource_name);
		$this->_debug_resource['basename'] = basename($resource_name);
	}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/rtp_smarty/class.tx_rtpsmarty_wrapper.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/rtp_smarty/class.tx_rtpsmarty_wrapper.php']);
}
